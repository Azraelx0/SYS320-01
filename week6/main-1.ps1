...
while($operation){
    ...
    elseif($choice -eq 3){ 
        ...
        #              - If true is returned, continue with the rest of the function
            if(checkPassword $password){
                ...
                Write-Host "Password does not meet requirements (6+ characters, 1 letter, 1 number, 1 special character.)"
            }
        }
     }    
    ...
    elseif($choice -eq 4){
        ...
        if(checkUser $name){
            ...
            Write-Host "User $name does not exist. Cannot remove."
        }
    }
    ...
    elseif($choice -eq 5){
        ...
        if(checkUser $name){
            ...
            Write-Host "User $name does not exist. Cannot enable."
        }
    }
    ...
    elseif($choice -eq 6){
        ...
        if(checkUser $name){
            ...
            Write-Host "User $name does not exist. Cannot disable."
        }
    }
    ...
    elseif($choice -eq 7){
        ...
        # TODO: Check the given username with the checkUser function.
        if(checkUser $name){
            $days = Read-Host -Prompt "Enter number of days to retrieve logs for"
            $userLogins = getLogInAndOffs $days
        # TODO: Change the above line in a way that, the days 90 should be taken from the user
            Write-Host ($userLogins | Where-Object { $_.User -ilike "*$name"} | Format-Table | Out-String)
         }
         else{
            Write-Host "User $name does not exist. Cannot retrieve logs."
        }
    }
    ...
    elseif($choice -eq 8){
        ...
        # TODO: Check the given username with the checkUser function.
        if(checkUser $name){
            $days = Read-Host -Prompt "Enter number of days to retrieve failed login attempts for"
            $userLogins = getFailedLogins $days
        # TODO: Change the above line in a way that, the days 90 should be taken from the user
            Write-Host ($userLogins | Where-Object { $_.User -ilike "*$name"} | Format-Table | Out-String)
        }
        else{
            Write-Host "User $name does not exist. Cannot retrieve failed login attempts."
        }
    }
    ...
    elseif($choice -eq 9){
       ...
       if($atRiskUsers.Count -gt 0){
           Write-Host "`nUsers at risk (more than 10 failed logins in last $days days):"
           Write-Host ($atRiskUsers | Select-Object Name,Count | Format-Table | Out-String)
       }
       else{
           Write-Host "No users at risk found in the last $days days."
       }
    }
    ...
    # Any choice that is not listed
    else{
        Write-Host "Invalid choice. Please select an option 1-10."
    }
  }
